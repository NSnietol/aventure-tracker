...
from aventure_tracker.models.flight import RouteConfig, RoutesConfig, WeekendTrip
...
def mock_scraper() -> AsyncMock:
    ...
    scraper = AsyncMock()
    scraper.get_cheapest_price = AsyncMock(return_value=120000)
    return scraper
...
class TestPriceAlert:
    ...
    def test_create_price_alert(self, route: RouteConfig) -> None:
        ...
        assert alert.current_price == 120000
        assert alert.price_change == -30000
        assert alert.price_change_pct == -20.0

    def test_should_notify_below_threshold(self, route: RouteConfig) -> None:
    ...
    def test_should_notify_significant_drop(self, route: RouteConfig) -> None:
        """Test should_notify is True for significant drop."""
        alert = PriceAlert(
            route=route,
            travel_date=date(2025, 3, 15),
            current_price=170000,  # Above threshold
    ...
    def test_should_notify_false(self, route: RouteConfig) -> None:
        """Test should_notify is False when conditions not met."""
        alert = PriceAlert(
            route=route,
            travel_date=date(2025, 3, 15),
            current_price=180000,  # Above threshold
...
class TestCreateAlert:
    ...
    def test_create_alert_below_threshold(
        ...
        assert alert.current_price == 100000
    ...
    def test_create_alert_with_previous_price(
        ...
        mock_state_manager.get_last_flight_price.return_value = 150000
        ...
        assert alert.previous_price == 150000
        assert alert.price_change == -30000
        assert alert.price_change_pct == -20.0
...
class TestCheckRoute:
    """Tests for checking individual routes."""

    @pytest.mark.asyncio
    async def test_check_route_returns_alert(
        self,
        service: FlightTrackerService,
        route: RouteConfig,
        mock_scraper: AsyncMock,
    ) -> None:
        """Test check_route returns PriceAlert."""
        mock_scraper.get_cheapest_price.return_value = 100000

        alert = await service.check_route(route, date(2025, 3, 15))
    ...
    @pytest.mark.asyncio
    async def test_check_route_returns_none_on_no_price(
        self,
        service: FlightTrackerService,
        route: RouteConfig,
        mock_scraper: AsyncMock,
    ) -> None:
        """Test check_route returns None when no price found."""
        mock_scraper.get_cheapest_price.return_value = None
    ...
    @pytest.mark.asyncio
    async def test_check_route_handles_error(
        self,
        service: FlightTrackerService,
        route: RouteConfig,
        mock_scraper: AsyncMock,
    ) -> None:
        """Test check_route handles scraper errors."""
        mock_scraper.get_cheapest_price.side_effect = Exception("Scraper failed")
...
class TestTrackFlights:
    ...
    async def test_track_flights_returns_result(
        self, service: FlightTrackerService
    ) -> None:
        ...
        assert result.routes_checked == 2  # 2 routes in config
    ...
    async def test_track_flights_generates_alerts(
        ...
        mock_scraper.get_cheapest_price.return_value = 100000  # Below threshold
    ...
    @pytest.mark.asyncio
    async def test_track_flights_handles_no_price(
        self,
        service: FlightTrackerService,
        mock_scraper: AsyncMock,
    ) -> None:
        """Test track_flights handles missing prices gracefully."""
        mock_scraper.get_cheapest_price.return_value = None
    ...
    async def test_track_flights_collects_errors(
        ...
        mock_scraper.get_cheapest_price.side_effect = Exception("Network error")
...
class TestUpdatePriceState:
    """Tests for state updates."""

    def test_update_price_state_calls_manager(
        self,
        service: FlightTrackerService,
        route: RouteConfig,
        mock_state_manager: MagicMock,
    ) -> None:
        """Test update_price_state calls state manager."""
        service._update_price_state(route, date(2025, 3, 15), 120000)

        mock_state_manager.set_flight_price.assert_called_once()
        call_args = mock_state_manager.set_flight_price.call_args
        assert call_args[0][1] == 120000

    def test_update_price_state_without_manager(
        self, routes_config: Path, route: RouteConfig
    ) -> None:
        """Test update_price_state handles no state manager."""
